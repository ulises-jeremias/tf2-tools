...
def load(config, datagen_flow=False, with_datasets=False):
    ...
    if config['data.dataset'] == "cifar10":
        (x_train, y_train), (x_val, y_val), (x_test,
                                             y_test) = load_cifar10(config['data.dataset'])
    elif config['data.dataset'] == "cifar100":
        (x_train, y_train), (x_val, y_val), (x_test,
                                             y_test) = load_cifar100(config['data.dataset'])
    elif config['data.dataset'] == "mnist":
        (x_train, y_train), (x_val, y_val), (x_test,
                                             y_test) = load_mnist(config['data.dataset'])
    ...
    if datagen_flow:
        ...
        train_gen = train_datagen.flow(
            x_train, y_train, batch_size=config['data.batch_size'])
        test_gen = test_datagen.flow(
            x_test, y_test, batch_size=config['data.batch_size'], shuffle=False)
